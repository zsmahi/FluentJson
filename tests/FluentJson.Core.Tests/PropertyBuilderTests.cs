...
public class PropertyBuilderTests
{
    ...
    [Fact]
    public void HasConversion_Should_UpdateMetadataFields()
    {
        // Arrange
        var builder = new PropertyBuilder<Guid>();
        var propertyInfo = typeof(Product).GetProperty(nameof(Product.Name))!; // dummy

        // Act
        builder.HasConversion<string>(g => g.ToString(), s => Guid.Parse(s));
        var result = ((IPropertyBuilder)builder).Build(propertyInfo, "Name");

        // Assert
        result.ConvertedType.Should().Be(typeof(string));
        result.SerializeFunc.Should().NotBeNull();
        result.DeserializeFunc.Should().NotBeNull();

        // Verify delegates
        var serialize = (Func<Guid, string>)result.SerializeFunc!;
        var deserialize = (Func<string, Guid>)result.DeserializeFunc!;

        var id = Guid.NewGuid();
        var str = serialize(id);
        str.Should().Be(id.ToString());

        var backToGuid = deserialize(str);
        backToGuid.Should().Be(id);
    }

    [Fact]
    public void HasConversion_Should_ThrowArgumentNullException_WhenDelegatesAreNull()
    {
        var builder = new PropertyBuilder<Guid>();

        Action act1 = () => builder.HasConversion<string>(null!, s => Guid.Parse(s));
        act1.Should().Throw<ArgumentNullException>().WithParameterName("serialize");

        Action act2 = () => builder.HasConversion<string>(g => g.ToString(), null!);
        act2.Should().Throw<ArgumentNullException>().WithParameterName("deserialize");
    }
}
